...
function(ArchiveFile=NULL, DraftOnly=FALSE, SendersEmail=NULL, Deadline=NULL, Salutation="Kind regards,\nAnne"){
  ...
  for(i in 1:length(unique(need.doi$Contact.Email))){
    titles <- need.doi$Title[need.doi$Contact.Email %in% unique(need.doi$Contact.Email)[i]]
    urls <- need.doi$X[need.doi$Contact.Email %in% unique(need.doi$Contact.Email)[i]]
